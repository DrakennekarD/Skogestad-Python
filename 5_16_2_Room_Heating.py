...
# I.e. your system will always be strong enough to counteract your
# disturbances.

# Using R = 3 you get an intersection of w_r = 0.0065 which implies that
# the plant will be able to track set point changes up to w_r. The required
# response time is 1/1000 = 0.001. Therefore w_r is better than the required
# response time.
...
# However, at about w = 0.004 rad/s you can see that setpoints will not be
# tracked that is, the error will be more than one.

plt.figure("Dead Time Constraint on cross over frequency")
L = G*K
scont.dead_time_bound(L, Gd, 100, freq=freqs)

# From the output you can see that because the upper bound of w_c is so close
# to the lower bound of w_c you are going to have a problem.
